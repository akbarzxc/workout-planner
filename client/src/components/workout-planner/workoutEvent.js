...
<div className="flex flex-row items-center text-slate-800 text-md space-x-2">
        ...
        <select
          ...
          value={selectedMovement ? selectedMovement.id : ""}
        >
          {movements.map((movement, index) => (
            <option key={movement.movement_id} value={index}>
              {movement.name}
            </option>
          ))}
        </select>
...
/*
  ...
  const handleSelectChange = (event) => {
    setSelectedMovement(movements[event.target.value]);
  };

  
  const AddMovement = async () => {
    const token = await getToken();
    const response = eventService.postWorkoutMovement(
      token,
      event.event_id,
      3,
      10,
    ...
    var updated = {
      ...event,
      muscle_groups: [...event.muscle_groups, response],
    };
    UpdateEventState(updated);
  };
  const addMovementMutation = useMutation(AddMovement);
  */
